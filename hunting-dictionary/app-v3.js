/* v0.3: preserve field-book layout; separate discovery tips from basic terms. */
(()=>{'use strict';
...
const all=D.entries,byId=new Map(all.map(e=>[e.id,e])),key='mh-hunting-dictionary:v1:bookmarks',wn={sa:'スラアク',bow:'弓',cb:'チャアク',gs:'大剣',sns:'片手剣'};
let selected=all[0]?.id||'',visible=[],onlyFavs=false,favs=new Set(),timer;
const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const norm=s=>String(s).normalize('NFKC').toLowerCase().replace(/[\u30a1-\u30f6]/g,c=>String.fromCharCode(c.charCodeAt(0)-0x60));
const complaintAliases={'flash-resistance-mr':['閃光が効かない','閃光がきかない','閃光きかない'],'radial-loadout':['ショトカが戻る','ショートカットが戻る'],'mantle-cooldown':['装衣が復活しない','装衣が戻らない']};
const hay=new Map(all.map(e=>[e.id,norm([e.title,e.reading,...e.aliases,e.quick,e.cue,e.action,e.caution,e.scope||'',e.route||'',e.category,...(complaintAliases[e.id]||[]),...e.weapons.map(w=>wn[w]||w)].join(' '))]));
...
function hasFilter(){return $('lookup').value||$('weapon').value!=='all'||$('category').value!=='all'||$('edition').value!=='all'||$('first').checked||onlyFavs||$('entry-mode').value!=='tips';}
function filter(){const terms=norm($('lookup').value.trim()).split(/\s+/).filter(Boolean),w=$('weapon').value,c=$('category').value,t=$('edition').value,k=$('entry-mode').value;
visible=all.filter(e=>(k==='all'||(k==='tips'?e.discovery:!e.discovery))&&(w==='all'||(w==='common'?!e.weapons.length:!e.weapons.length||e.weapons.includes(w)))&&(c==='all'||e.category===c)&&(t==='all'||e.edition===t)&&(!$('first').checked||e.first)&&(!onlyFavs||favs.has(e.id))&&terms.every(q=>hay.get(e.id).includes(q)));
...
$('detail').innerHTML=`<button class="bookmark" id="bookmark" aria-label="${favs.has(e.id)?'しおりを外す':'しおりに挟む'}" title="${favs.has(e.id)?'しおりを外す':'しおりに挟む'}" aria-pressed="${favs.has(e.id)}"><svg viewBox="0 0 28 50" aria-hidden="true"><path d="M2 0H26V46L14 37 2 46Z"/><path d="M9 10H19M9 16H19" fill="none" stroke="currentColor" stroke-width="1"/></svg></button><div class="entryhead"><div><div class="eyebrow">${esc(e.weapons.length?e.weapons.map(w=>wn[w]).join('・'):e.category)}${e.edition==='iceborne'?'<span class="ib">IB</span>':''}</div><h2>${esc(e.title)}</h2></div></div><p class="takeaway">${esc(v.short)}</p>${e.scope?'<p class="tip-scope">'+esc(e.scope)+'</p>':''}<div class="story" id="story">${panels}</div><p class="warning"><span class="mark" aria-hidden="true">!</span><span>${esc(v.warning)}</span></p>${e.route?'<p class="tip-route">'+esc(e.route)+'</p>':''}<details class="more"><summary>詳しく・出典</summary><div class="detailtext"><p><b>場面</b>　${esc(e.cue)}</p><p><b>実践</b>　${esc(e.action)}</p><p><b>注意</b>　${esc(e.caution)}</p>${e.note?'<p>'+esc(e.note)+'</p>':''}<p><b>別名・困りごと</b>　${e.aliases.map(esc).join(' / ')}</p>${e.sources.map(k=>D.sources[k]).filter(Boolean).map(s=>`<a href="${esc(s.url)}" target="_blank" rel="noopener noreferrer">${esc(s.label)} ↗</a>${s.note?'<p>'+esc(s.note)+'</p>':''}`).join('')}<small>資料確認：${e.reviewed||D.reviewed}。ゲーム内での再現検証は未実施。図・記号は模式表現で、ゲーム画面ではありません。</small></div></details><nav class="relations" aria-label="関連項目">${e.related.filter(id=>byId.has(id)).map(id=>`<button data-related="${id}">${V.icon(id)}<span>${esc(byId.get(id).title)} ↗</span></button>`).join('')}</nav><div class="paper-bottom"><span>${String(all.indexOf(e)+1).padStart(2,'0')} / ${all.length}</span><button class="copy" id="copy">項目のリンクをコピー</button></div>`;
...
function reset(mode='tips'){ $('lookup').value='';for(const x of ['weapon','category','edition'])$(x).value='all';$('first').checked=false;onlyFavs=false;$('entry-mode').value=mode;filter();}
function go(id,push=true){if(!byId.has(id)){toast('この項目はまだ収録されていません。');return;}if(!visible.some(e=>e.id===id))reset('all');selected=id;renderList();renderDetail();view('detail');$('filterdetails').open=false;if(push&&location.hash.slice(1)!==id)window.history.pushState(null,'','#'+id);if(matchMedia('(max-width:850px)').matches)$('detail').scrollIntoView({block:'start'});$('detail').focus({preventScroll:true});}
[...new Set(all.map(e=>e.category))].forEach(c=>{const o=document.createElement('option');o.value=c;o.textContent=c;$('category').append(o);});
// A typed lookup starts across the entire dictionary; users can narrow it afterwards.
$('lookup').addEventListener('input',()=>{if($('lookup').value.trim())$('entry-mode').value='all';filter();view('index');});$('clear').onclick=()=>{$('lookup').value='';filter();view('index');$('lookup').focus();};
for(const id of ['weapon','category','edition','first','entry-mode'])$(id).onchange=()=>{filter();view('index');};
$('reset').onclick=()=>{reset();view('index');};$('only-favs').onclick=()=>{onlyFavs=!onlyFavs;if(onlyFavs)$('entry-mode').value='all';filter();view('index');};
$('entries').onclick=e=>{const b=e.target.closest('button[data-id]');if(b)go(b.dataset.id);if(e.target.closest('[data-reset]')){reset('all');view('index');}};
...
function hashRead(){let id;try{id=decodeURIComponent(location.hash.slice(1));}catch(_){toast('項目リンクの形式が不正です。');return;}if(id==='lookup')return;if(id)go(id,false);else if(all.length)go(all[0].id,false);}
...
$('export').onclick=()=>{const blob=new Blob([JSON.stringify({schema:'mh-hunting-dictionary-bookmarks-v1',version:D.version,exported:new Date().toISOString(),ids:[...favs]},null,2)],{type:'application/json'});const u=URL.createObjectURL(blob),a=document.createElement('a');a.href=u;a.download='hunting-dictionary-bookmarks.json';document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(u),1000);toast('しおりを書き出しました。');};
