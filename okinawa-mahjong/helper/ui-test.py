...
with sync_playwright() as pw:
    ...
    check('shared glossary badges',page.locator('.term-chip').count()==9)
    page.locator('[data-term="call"]').click();expect(page.locator('#term-note')).to_contain_text('リーチできない');check('call glossary opens inline')
    page.locator('[data-term="fu"]').click();expect(page.locator('#term-note')).to_contain_text('手の形につく');check('fu glossary beginner wording')
    page.locator('[data-term="kan"]').click();expect(page.locator('#term-note')).to_contain_text('暗カン');expect(page.locator('#term-note')).to_contain_text('明カン');check('kan glossary covers open closed distinction')
    ...
    expect(page.locator('#paper-score')).to_be_visible();expect(page.locator('#detail-score')).to_be_hidden();check('paper score is beginner default')
    expect(page.locator('#paper-score')).to_contain_text('300/500');expect(page.locator('#paper-score')).to_contain_text('500オール');check('paper table child/dealer tsumo')
    expect(page.locator('#paper-score')).to_contain_text('符ってなに？');check('paper score explains fu')
    page.locator('[data-score-mode="detail"]').click();expect(page.locator('#detail-score')).to_be_visible();check('detail score switch')
